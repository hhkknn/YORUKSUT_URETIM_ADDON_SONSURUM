...
{
    public class UretimSiparisiIptal
    {
        [ItemAtt(AIFConn.UretimSiparisiIptalUID)]
        public SAPbouiCOM.Form frmUretimSiparisiIptal;

        [ItemAtt("Item_2")]
        public SAPbouiCOM.EditText oEdtSatirSayisi;
        [ItemAtt("Item_0")]
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.UretimSiparisiIptalXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.UretimSiparisiIptalXML));
            Functions.CreateUserOrSystemFormComponent<UretimSiparisiIptal>(AIFConn.UrtmIpt);
                ...
                case BoEventTypes.et_CLICK:
                    if (pVal.ItemUID == "Item_4" && !pVal.BeforeAction)
                    {
                        for (int i = 1; i <= oMatrix.RowCount; i++)
                        {
                            ...
                            var docEntry = ((SAPbouiCOM.EditText)oMatrix.Columns.Item("Col_0").Cells.Item(i).Specific).Value;
                            wo.GetByKey(Convert.ToInt32(docEntry));
                            wo.ProductionOrderStatus = BoProductionOrderStatusEnum.boposCancelled;
                            wo.Update();
                    ...
                    {
                        var SatirSayisi = oEdtSatirSayisi.Value.ToString();
                        oMatrix.Clear();
                        oMatrix.AddRow(Convert.ToInt32(SatirSayisi));
