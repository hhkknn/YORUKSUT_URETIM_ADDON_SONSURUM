...
{
    public class IndirimUrunEkle
    {
        [ItemAtt(AIFConn.IndirimUrunEkleUID)]
        public static SAPbouiCOM.Form frmIndirimUrunEkle;
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.IndirimUrunEkleFrmXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.IndirimUrunEkleFrmXML));
            Functions.CreateUserOrSystemFormComponent<IndirimUrunEkle>(AIFConn.UrunEkle);
            guncellemeModu = _guncellemeModu;

            if (guncellemeModu != "")
            {
                frmIndirimUrunEkle.Title = "Ürün Güncelle";
            }
            InitForms();
        }
        string sql = "";
        ...
        public void InitForms()
        {
            try
            {
                frmIndirimUrunEkle.Freeze(true);
                oDataTable = frmIndirimUrunEkle.DataSources.DataTables.Add("DATA");
                //oMatrix.AddRow();

                frmIndirimUrunEkle.EnableMenu("1283", false);
                frmIndirimUrunEkle.EnableMenu("1284", false);
                frmIndirimUrunEkle.EnableMenu("1286", false);
                ...
                {
                    SAPbouiCOM.Matrix oMatrix = (SAPbouiCOM.Matrix)IndirimGiris.frmIndirimGiris.Items.Item("Item_8").Specific;
            ...
            {
                frmIndirimUrunEkle.Freeze(false);
            }
        }
        List<UrunEkle> urunEkles = new List<UrunEkle>();
        ...
        public bool SAP_ItemEvent(string FormUID, ref ItemEvent pVal, ref bool BubbleEvent)
        {
            ...
            switch (pVal.EventType)
            {
                ...
                case BoEventTypes.et_CLICK:
                    if (!pVal.BeforeAction && pVal.ItemUID == "Item_10")
                    {
                        ...
                        try
                        {
                            ...
                            List<UrunEkle> urunEkleArama = null;
                    ...
                    {
                        List<UrunEkle> listeGonder = urunEkles.Where(c => c.Sec == "Y").ToList();
                        ...
                        //double _besinciIsk = Convert.ToDouble(oEditBesinciIsk.Value, System.Globalization.CultureInfo.InvariantCulture);
                        string _toplamIsk = oEditToplamIsk.Value == null ? Convert.ToDouble("0").ToString() : oEditToplamIsk.Value;

                        if (guncellemeModu == "")
                        {
                            AIFConn.IndrmGrs.UrunleriEkle(listeGonder, _birinciIsk, _ikinciIsk, _ucuncuIsk, _dorduncuIsk, _besinciIsk, _toplamIsk);
                        }
                        else if (guncellemeModu != "")
                        {
                            AIFConn.IndrmGrs.UrunleriGuncelle(listeGonder, _birinciIsk, _ikinciIsk, _ucuncuIsk, _dorduncuIsk, _besinciIsk, _toplamIsk);
                        }
                        try
                        {
                            frmIndirimUrunEkle.Close();
                        }
                        catch (Exception)
                        {
                        }
                ...
                case BoEventTypes.et_VALIDATE:
                    ...
                    {
                        double _birinciIsk = Convert.ToDouble(oEditBirinciIsk.Value, System.Globalization.CultureInfo.InvariantCulture);
                        double _ikinciIsk = Convert.ToDouble(oEditIkinciIsk.Value, System.Globalization.CultureInfo.InvariantCulture);
                        double _ucuncuIsk = Convert.ToDouble(oEditUcuncuIsk.Value, System.Globalization.CultureInfo.InvariantCulture);
                        double _dorduncuIsk = Convert.ToDouble(oEditDorduncuIsk.Value, System.Globalization.CultureInfo.InvariantCulture);
                        double _besinciIsk = Convert.ToDouble(oEditBesinciIsk.Value, System.Globalization.CultureInfo.InvariantCulture);
