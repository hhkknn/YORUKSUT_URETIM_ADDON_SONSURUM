...
namespace AIF.UVT.SAPB1.ClassLayer
{
    ...
    {
        [ItemAtt(AIFConn.AletEkipmanTemizlikUID)]
        public SAPbouiCOM.Form frmAleyEkipmanTemizlik;

        [ItemAtt("Item_4")]
        public SAPbouiCOM.Matrix oMatrix;
        [ItemAtt("Item_1")]
        public SAPbouiCOM.ComboBox oComboIstasyon;
        [ItemAtt("Item_3")]
        public SAPbouiCOM.EditText oEdtIstasyon;
        [ItemAtt("Item_8")]
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.AletEkipmanTemizlikXML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.AletEkipmanTemizlikXML));
            Functions.CreateUserOrSystemFormComponent<AletEkipmanTemizlik>(AIFConn.AETTem);
        ...
        public void InitForms()
        {
            try
            {
                ...
                string sql = "select T1.FldValue,T1.Descr from CUFD as T0 INNER JOIN UFD1 as T1 ON T0.FieldID = T1.FieldID where T0.TableID = 'OWOR' and T1.TableID = 'OWOR' and T0.AliasID='ISTASYON'";
                ...
                {
                    oComboIstasyon.ValidValues.Add(ConstVariables.oRecordset.Fields.Item(0).Value.ToString(), ConstVariables.oRecordset.Fields.Item(1).Value.ToString());
            ...
            switch (pVal.EventType)
            {
                ...
                case BoEventTypes.et_COMBO_SELECT:
                    if (pVal.ItemUID == "Item_1" && !pVal.BeforeAction)
                    {
                        try
                        {
                            if (frmAleyEkipmanTemizlik.Mode != BoFormMode.fm_FIND_MODE)
                            {
                                oEdtIstasyon.Value = oComboIstasyon.Selected.Value;

                                string sql = "select \"DocEntry\" from \"@AIF_AET\" where \"U_IstasyonKodu\" = '" + oEdtIstasyon.Value + "'";
                                ConstVariables.oRecordset = (SAPbobsCOM.Recordset)ConstVariables.oCompanyObject.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                                ...
                                if (ConstVariables.oRecordset.RecordCount > 0)
                                {
                                    frmAleyEkipmanTemizlik.Mode = BoFormMode.fm_FIND_MODE;
                                    oEdtDocEntry.Value = ConstVariables.oRecordset.Fields.Item(0).Value.ToString();
                                    btnAddOrUpdate.Item.Click();
                                }
            ...
            try
            {
                if (pVal.MenuUID == "AIFRGHTCLK_DeleteRow" && pVal.BeforeAction)
                {
                    ...
                    if (row != -1)
                    {
                        ...
                        if (frmAleyEkipmanTemizlik.Mode == BoFormMode.fm_OK_MODE)
                        {
                            frmAleyEkipmanTemizlik.Mode = BoFormMode.fm_UPDATE_MODE;
                        }
                    }
                }
                else if (pVal.MenuUID == "AIFRGHTCLK_AddRow" && pVal.BeforeAction)
                {
                    frmAleyEkipmanTemizlik.DataSources.DBDataSources.Item(1).Clear();
                    oMatrix.AddRow();
