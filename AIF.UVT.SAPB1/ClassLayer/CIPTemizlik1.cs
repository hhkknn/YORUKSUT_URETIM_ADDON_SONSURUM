...
{
    public class CIPTemizlik1
    {
        [ItemAtt(AIFConn.CIPTemizlik1UID)]
        public SAPbouiCOM.Form frmCIPTemizlik1;
        ...
        {
            ConstVariables.oFnc.LoadSAPXML(AIFConn.CIPTemizlik1XML, Assembly.GetExecutingAssembly().GetManifestResourceStream(AIFConn.CIPTemizlik1XML));
            Functions.CreateUserOrSystemFormComponent<CIPTemizlik1>(AIFConn.CIPTem1);
            ...
            switch (pVal.EventType)
            {
                ...
                case BoEventTypes.et_COMBO_SELECT:
                    if (pVal.ItemUID == "Item_1" && !pVal.BeforeAction)
                    {
                        try
                        {
                            if (frmCIPTemizlik1.Mode != BoFormMode.fm_FIND_MODE)
                            {
                                oEdtIstasyon.Value = oComboIstasyon.Selected.Value;

                                string sql = "select \"DocEntry\" from \"@AIF_CIP2\" where \"U_IstasyonKodu\" = '" + oEdtIstasyon.Value + "'";
                                ConstVariables.oRecordset = (SAPbobsCOM.Recordset)ConstVariables.oCompanyObject.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                                ...
                                if (ConstVariables.oRecordset.RecordCount > 0)
                                {
                                    frmCIPTemizlik1.Mode = BoFormMode.fm_FIND_MODE;
            ...
            BubbleEvent = true; try
            {
                if (pVal.MenuUID == "AIFRGHTCLK_DeleteRow" && pVal.BeforeAction)
                {
                    ...
                    if (row != -1)
                    {
                        ...
                        if (frmCIPTemizlik1.Mode == BoFormMode.fm_OK_MODE)
                        {
                            frmCIPTemizlik1.Mode = BoFormMode.fm_UPDATE_MODE;
                        }
                    }
                }
                else if (pVal.MenuUID == "AIFRGHTCLK_AddRow" && pVal.BeforeAction)
                {
                    frmCIPTemizlik1.DataSources.DBDataSources.Item(1).Clear();
                    oMatrix.AddRow();
